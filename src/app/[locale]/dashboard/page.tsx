    ...
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <div className="max-w-7xl mx-auto px-3 pb-2">
        ...
        <div className="max-w-7xl mx-auto px-3 pb-8">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-white mb-2">📊 Economic Regime Investment Portfolios</h2>
          ...
          {economicPortfolios ? (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {(() => {
                  console.log('🏦 Rendering economic portfolios:', economicPortfolios ? Object.keys(economicPortfolios).length : 0);
                  console.log('🏦 Portfolio data structure:', economicPortfolios);
                  ...
                  .map(([key, portfolio]: [string, any], index: number) => {
                    console.log('📊 Main Portfolio rendering:', { key, portfolio, index });
                    
                    // Defensive check for portfolio properties
                    if (!portfolio || typeof portfolio !== 'object') {
                      console.warn('⚠️ Invalid portfolio object:', portfolio);
                      return null;
                    }
                    ...
                    <div key={key} className={`bg-slate-800 rounded-xl p-6 border-2 transition-all duration-300 hover:shadow-lg hover:scale-[1.02] relative ${
                      ...
                      <div className="mb-6">
                        ...
                        <div className="mb-4">
                          <div className="text-sm font-semibold text-gray-300 mb-3">📊 Performance Analysis</div>
                        ...
                        <div className={`p-3 rounded-lg border-l-4 ${
                          ...
                          <div className="text-xs font-semibold mb-1">
                            {parseFloat(portfolio.avgPerformance) > 1 ? '🟢 Strong Buy Signal' :
                             parseFloat(portfolio.avgPerformance) > 0 ? '🔵 Buy Signal' :
                             parseFloat(portfolio.avgPerformance) > -1 ? '🟡 Hold/Caution' :
                             '🔴 Avoid/Reduce'}
                          </div>
...
🎯 Economic Regime: ${key.replace(/([A-Z])/g, ' $1').trim()}

� MULTI-TIMEFRAME PERFORMANCE:
${portfolio.avgPerformances ? `
• Daily:     ${portfolio.avgPerformances.daily > 0 ? '+' : ''}${portfolio.avgPerformances.daily.toFixed(2)}%
• Weekly:    ${portfolio.avgPerformances.weekly > 0 ? '+' : ''}${portfolio.avgPerformances.weekly.toFixed(2)}%
• Monthly:   ${portfolio.avgPerformances.monthly > 0 ? '+' : ''}${portfolio.avgPerformances.monthly.toFixed(2)}%
• Quarterly: ${portfolio.avgPerformances.quarterly > 0 ? '+' : ''}${portfolio.avgPerformances.quarterly.toFixed(2)}%
• Yearly:    ${portfolio.avgPerformances.yearly > 0 ? '+' : ''}${portfolio.avgPerformances.yearly.toFixed(2)}%
` : `Current: ${portfolio.avgPerformance}`}

📈 Risk Level: ${Math.abs(parseFloat(portfolio.avgPerformance)) > 2 ? 'High' : Math.abs(parseFloat(portfolio.avgPerformance)) > 1 ? 'Medium' : 'Low'}

💼 DETAILED HOLDINGS (${portfolio.etfData ? portfolio.etfData.length : 0} ETFs):
...
📈 Market Context: ${portfolio?.description || 'Portfolio description not available'}

💡 PROFESSIONAL ANALYSIS:
• This portfolio is designed for ${portfolio?.description ? portfolio.description.toLowerCase() : 'market conditions'}
• Diversification across ${portfolio?.etfData ? portfolio.etfData.length : 0} ETFs reduces single-asset risk
• Rebalance quarterly or when allocations drift >5% from targets
• Monitor economic indicators that affect this regime
                              `;
                              ...
                              modal.innerHTML = `
                                <div class="bg-slate-800 rounded-xl p-6 max-w-4xl max-h-[85vh] overflow-y-auto m-4 border border-slate-600 shadow-2xl">
                                  <div class="flex justify-between items-center mb-4 sticky top-0 bg-slate-800 z-10 pb-2">
                                    <h3 class="text-xl font-bold text-white">📊 Portfolio Detailed Analysis</h3>
...
💰 HOW TO INVEST IN ${portfolio.name ? portfolio.name.toUpperCase() : key.replace(/([A-Z])/g, ' $1').trim().toUpperCase()}

🎯 Portfolio Strategy: ${key.replace(/([A-Z])/g, ' $1').trim()}
📊 Current Performance: ${portfolio.avgPerformance || 'N/A'}

📋 STEP-BY-STEP INVESTMENT PROCESS:

1️⃣ PREPARATION
   • Assess your risk tolerance
   • Determine investment amount (recommend 5-15% of portfolio)
   • Current risk level: ${portfolio.avgPerformance ? (Math.abs(parseFloat(portfolio.avgPerformance)) > 2 ? 'HIGH' : Math.abs(parseFloat(portfolio.avgPerformance)) > 1 ? 'MEDIUM' : 'LOW') : 'N/A'}

2️⃣ BROKER SELECTION
   • Choose a broker: Fidelity, Vanguard, Charles Schwab, E*TRADE
   • Ensure they offer ETF trading
   • Look for commission-free ETF trades
...
${portfolio.etfData && Array.isArray(portfolio.etfData) ? portfolio.etfData.map((etf: any, idx: number) => {
  ...
  return `   ${idx + 1}. ${etf.ticker || 'N/A'} (${allocation}%) - Current Price: $${etf.price || '0.00'}
      • ${etf.name || 'ETF Name'}
      • Performance: ${etf.performance ? (etf.performance > 0 ? '+' : '') + etf.performance.toFixed(2) + '%' : 'N/A'}`;
}).join('\n') : '   • Equal weight distribution across all holdings'}

4️⃣ EXECUTION TIMELINE
   • Buy in 2-3 tranches over 4-6 weeks (dollar-cost averaging)
   • Set up automatic rebalancing quarterly
   • Monitor monthly for major regime changes

5️⃣ REBALANCING STRATEGY
   • Review quarterly
   • Rebalance if any ETF deviates >5% from target
   • Adjust based on economic regime changes
...
💡 PRO TIPS:
   • Start with 50% of intended allocation
   ...
   • Track performance vs benchmark (SPY)
                              ...
                              modal.innerHTML = `
                                <div class="bg-slate-800 rounded-xl p-6 max-w-3xl max-h-[85vh] overflow-y-auto m-4 border border-slate-600">
                                  <div class="flex justify-between items-center mb-4">
                                    <h3 class="text-xl font-bold text-white">💰 Investment Guide</h3>
        ...
        {/* FedWatch Section */}
        <div className="max-w-7xl mx-auto px-3 pb-8">
          <h2 className="text-xl font-bold text-white mb-4 mt-8">🏦 US FedWatch - Rate Probability Analysis</h2>
        ...
        <div className="max-w-7xl mx-auto px-3 pb-8">
          <h2 className="text-xl font-bold text-white mb-4 mt-8">📊 Economic Regime Portfolios</h2>
        ...
        <div className="max-w-7xl mx-auto px-3 pb-8">
          <h2 className="text-xl font-bold text-white mb-6 mt-8">📅 Economic Calendar</h2>
          ...
          <div className="bg-slate-800 rounded-lg p-6">
            ...
            <div className="text-center text-xs text-gray-500 mt-6 pt-6 border-t border-slate-700">
              Times shown in EST • Data provided for educational purposes
            </div>
          </div>
        </div>
        </>
        ...
        <div className="max-w-7xl mx-auto px-3 pb-8">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-white mb-2">🏆 Top Wall Street Analysts</h2>
            <p className="text-gray-300 text-sm mb-4">
              ...
              {analystsData?.data?.topAnalysts?.map((analyst: any, index: number) => (
                ...
                <div key={analyst.name} className={`bg-slate-800 rounded-xl p-6 border-2 transition-all duration-300 hover:shadow-lg hover:scale-[1.02] relative ${
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-gray-400 text-xs mt-1">👥 {analyst.followers} followers</p>
                  </div>
                  ...
                  <div className={`p-3 rounded-lg border-l-4 text-center ${
                    ...
                    <div className="text-xs opacity-80">
                      {analyst.successRate > 85 ? '🏆 Elite Performance' :
                       analyst.successRate > 80 ? '🌟 Top Tier' :
                       '📈 Consistent'} 
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 mb-8">
            <h3 className="text-xl font-bold text-white mb-6">🔥 Recent Analyst Calls & Price Targets</h3>
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    ...
                    <div>
                      {call.oldPT !== "N/A" && call.oldPT !== call.newPT && (
                        <span>PT: {call.oldPT} → {call.newPT}</span>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-gradient-to-r from-indigo-900/20 to-purple-900/20 border border-indigo-500/30 rounded-xl p-6">
            <h3 className="text-lg font-bold text-white mb-4 flex items-center">
              📊 How We Track Analyst Performance
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="flex items-start space-x-3">
        ...
        <div className="max-w-7xl mx-auto px-3 pb-8">
          <h2 className="text-xl font-bold text-white mb-6 mt-8">📈 Markets Overview</h2>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              {stock.direction && (
                                <span className={`text-xs ${
                                  stock.direction === '↗' ? 'text-green-400' :
                                  stock.direction === '↘' ? 'text-red-400' :
                                  'text-yellow-400'
                                }`}>
                                  {stock.direction}
                                </span>
          ...
          {economicPortfolios && Object.keys(economicPortfolios).length > 0 && (
            <div className="mt-8">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-white">📊 Economic Regime Portfolios</h2>
                <button
                  onClick={() => setSelectedView('portfolios')}
                  ...
                  .map(([key, portfolio]: [string, any], index: number) => (
                    <div key={key} className={`bg-slate-800 rounded-lg p-4 border-l-4 ${
                      ...
                      <div className={`text-center py-2 rounded text-xs font-semibold ${
                        ...
                        {parseFloat(portfolio.avgPerformance) > 1 ? '🟢 Strong Buy' :
                         parseFloat(portfolio.avgPerformance) > 0 ? '🔵 Buy Signal' :
                         parseFloat(portfolio.avgPerformance) > -1 ? '🟡 Hold' :
                         '🔴 Avoid/Reduce'}
                      </div>
                    </div>
                  ))}
              ...
              <div className="text-center mt-4">
                <p className="text-xs text-gray-500">
                  Showing top 3 performing portfolios • Real-time data
                </p>
              </div>
            </div>
          )}
        ...
        <div className="max-w-7xl mx-auto px-3 pb-8">
          {spxValuation ? (
            <div>
              <h2 className="text-xl font-bold text-white mb-6 mt-8">📊 S&P 500 P/E Valuation Analysis</h2>
        ...
        <div className="max-w-7xl mx-auto px-3 pb-8">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-white mb-2">📈 Earnings Calendar</h2>
            <p className="text-gray-300 text-sm mb-4">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
                ...
                {earningsData.summary.nextMajorEarning && (
                  <div className="mt-6 bg-gradient-to-r from-blue-900/20 to-indigo-900/20 border border-blue-500/30 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="text-blue-300 font-bold text-sm">🎯 Next Major Earning</h4>
              ...
              <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                <div className="p-6 border-b border-slate-700">
                  <h3 className="text-lg font-bold text-white">📅 Upcoming Earnings</h3>
