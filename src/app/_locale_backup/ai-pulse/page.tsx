...
export default function AIPulsePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const fetchSectorData = async () => { try { setRefreshing(true); const r = await fetchT(`/api/sector-performance?period=${encodeURIComponent(selectedPeriod)}`, 12000, { cache:'no-store' }); if(!r.ok) throw new Error('sector'); const j = await r.json(); const sectors = j.sectors||[]; setSectorData(sectors); setDataHealth(h=>({...h,sector:sectors.length>0})); setLastUpdated(j.lastUpdated || new Date().toISOString()); setError(''); } catch(e){ console.error(e); setError('Failed to load sector performance data'); } finally { setLoading(false); setRefreshing(false);} };
  ...
  <div className="min-h-screen bg-[var(--background)] text-white">
        <div className="container mx-auto px-4 py-8">
          ...
          {/* Sector Performance simplified: only Best/Worst Stocks */}
          <div className="bg-gradient-to-br from-slate-800/70 via-slate-900/70 to-black/60 rounded-xl p-6 border border-white/10 mb-8">
            {(topMovers.length>0 || bottomMovers.length>0) ? (
              ...
              <div className="bg-white/5 rounded-lg p-4">
                <h4 className="text-white font-semibold mb-3">Best/Worst Stocks</h4>
                <div className="w-full h-20 animate-pulse bg-white/5 rounded" />
              </div>
            )}
          </div>
