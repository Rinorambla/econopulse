  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white flex items-center justify-center">
        <div className="text-center">
          {/* Professional spinner with glow */}
          <div className="relative mx-auto mb-6">
            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 blur-xl opacity-30 animate-pulse"></div>
            <div className="relative animate-spin rounded-full h-16 w-16 border-2 border-transparent border-t-cyan-400 border-r-blue-400 mx-auto"></div>
            <div className="absolute inset-2 rounded-full bg-gradient-to-br from-cyan-500/20 to-blue-500/20"></div>
          </div>
          <p className="text-lg font-medium text-gray-200 mb-2">Visual AI Dashboard</p>
          <p className="text-sm text-gray-400">Loading market intelligence...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
        ...
        <div className="relative container mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-12">
            {widgets
              // Country-level widgets (population/debt/GDP/PMI/yields/trade) are
              // now layers of the Visual AI map above — hide the standalone cards.
              .filter(w => !['population', 'debt', 'gdp', 'pmi', 'yields', 'trade', 'population_fallback', 'debt_fallback', 'pmi_fallback', 'yields_fallback', 'trade_fallback'].includes(w.id))
              ...
              <div 
                key={widget.id} 
                className="group relative bg-gradient-to-br from-slate-800/50 via-slate-800/30 to-slate-900/50 backdrop-blur-sm rounded-2xl p-6 border border-white/10 hover:border-cyan-500/30 transition-all duration-500 hover:shadow-xl hover:shadow-cyan-500/5 hover:-translate-y-1"
              >
                ...
                <div className="relative flex items-start justify-between mb-4 gap-3">
                  <div>
                    <h3 className="text-lg font-semibold text-white group-hover:text-cyan-50 transition-colors">{widget.title}</h3>
                    <div className="mt-1 text-xs text-gray-500">Updated {new Date(widget.lastUpdated).toLocaleTimeString('en-US', {hour: '2-digit', minute: '2-digit'})}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {widget.fallback && (
                      <span className="px-2.5 py-1 rounded-lg text-[10px] font-medium border border-amber-500/30 text-amber-400 bg-amber-500/10">Partial Data</span>
                    )}
                    <div className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold flex items-center gap-1 ${getSentimentColor(widget.aiInsight.sentiment)}`}>
                      <Brain className="w-3 h-3" />
                      {widget.aiInsight.confidence}%
                    </div>
                  </div>
                </div>

                {/* Widget Visualization */}
                <div className="relative h-52 mb-4 bg-slate-900/30 rounded-xl p-2">
                ...
                <div className="relative flex items-start justify-between gap-4 p-4 -mx-2 rounded-xl bg-gradient-to-r from-sky-500/5 to-blue-500/5 border border-sky-500/10">
                  ...
                  <button
                    ...
                    className="shrink-0 inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-sm font-medium text-white transition-all duration-300 hover:scale-105"
                  >
        ...
        {selectedWidget && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50">
            <div className={`bg-gradient-to-br from-slate-900 via-slate-800/95 to-slate-900 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl shadow-black/50 w-full max-h-[95vh] flex flex-col ${
              ...
              <div className="flex items-center justify-between p-6 pb-4 border-b border-white/10">
                <div className="flex items-center gap-4">
                  ...
                  <h2 className="text-2xl font-bold bg-gradient-to-r from-white to-gray-300 text-transparent bg-clip-text">{selectedWidget.title}</h2>
                </div>
                <button
                  onClick={() => setSelectedWidget(null)}
                  className="text-gray-400 hover:text-white p-2.5 rounded-xl hover:bg-white/10 transition-all duration-300 border border-transparent hover:border-white/10"
                >
