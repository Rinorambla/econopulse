...
export async function GET() {
  try {
    ...
    if (marketData && marketData.length > 0) {
      ...
      if (spyData) {
        const spyChange = 'data' in spyData && (spyData as any).data ? (spyData as any).data.changePercent : (spyData as any).changePercent;
        if (typeof spyChange === 'number') fearGreedIndex += spyChange * 6; // SPY performance
      }
      
      if (vixData) {
        const vixChange = 'data' in vixData && (vixData as any).data ? (vixData as any).data.changePercent : (vixData as any).changePercent;
        if (typeof vixChange === 'number') fearGreedIndex -= vixChange * 2; // VIX inverse
      }
    }
