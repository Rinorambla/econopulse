...
// Koyfin-style quote page: chart + key stats + news + earnings for one symbol.
...
const AdvancedChart = dynamic(() => import('@/components/analytics/AdvancedChart'), {
  ssr: false,
  loading: () => (
    <div className="w-full h-[520px] bg-slate-900/40 border border-white/10 rounded-lg flex items-center justify-center text-xs text-gray-400">
      Loading chart…
    </div>
  ),
});
    ...
    <RequirePlan min="free">
      <TerminalShell title={symbol}>
        <div className="p-3 space-y-3 text-white">
          ...
          {/* Chart + stats */}
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_290px] gap-3">
            <div className="min-w-0">
              <AdvancedChart symbol={symbol} height={520} />
            </div>
