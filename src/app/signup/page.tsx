...
export default function SignupPage() {
  ...
  const handleApple = async () => {
    ...
    try {
      ...
      if (!res.success) {
        setError(res.error || 'Apple sign-up failed');
      }
    } catch (err) {
      ...
      <div className="w-full max-w-sm bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 shadow-xl">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-black hover:bg-gray-900 disabled:opacity-50 text-white font-semibold py-2.5 rounded-lg transition-colors"
          >
